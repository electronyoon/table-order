...
- **품목 취소는 두 종류**: 일반 탭 = 변심취소(`order_item.status = CANCELED`만), 길게 누르기 = 재고부족취소(`CANCELED` + `menu.sold_out_date = 오늘 영업일`을 한 트랜잭션으로). API는 `POST /admin/order-items/{id}/cancel`에 `markSoldOut` boolean으로 구분한다. 품절 판정은 `sold_out_date == 오늘 영업일`이므로 날짜가 지나면 자동 해제된다 (별도 배치 불필요).
