...
- 후불(카운터 결제): 결제는 **세션 단위**로 정산 → `Payment.session_id`
- 선불(QR 즉시 결제): 결제는 **주문 단위** → `Payment.order_id`
...
- 영업일 계산: 새벽 영업 대비 컷오프 시각(기본 06:00)을 두고, 06시 이전이면 전날을 영업일로 간주.
...
### payment — v0.1과 동일 (session_id / order_id 중 하나, method, status, amount, pg_* 예약)
...
### 품목 취소 → 당일 품절 (핵심 규칙)
1. 품목 [취소] 탭 → 모달: **"'제육볶음'을 취소하고 오늘 하루 품절 처리할까요?"** [품절 처리하고 취소] / [돌아가기]
2. 확정 시 한 트랜잭션으로:
...
4. 취소 사유 입력 없음. 시스템상 취소 = 재고 소진.

### 열린 질문 (구현 전 결정 필요)
손님 변심/오주문 취소("잘못 시켰어요")도 현실에서 발생한다. 이 경우 품절 처리는 오작동이 된다. 선택지:
- (a) 그대로 간다. 오품절 시 메뉴판에서 품절 배지 탭 → 해제 (액션 1번 추가로 복구 가능)
- (b) 취소 버튼 **길게 누르기** = 품절 없이 취소 (화면에 버튼을 늘리지 않고 숨은 경로 제공, 아들이 알려드리면 됨)
현재 문서는 (a) 기준. (b)는 태블릿 앱에서 저비용으로 추가 가능.

---

## 4. 결제 상태 (주문 상태와 분리) — v0.1과 동일

- 후불: 세션 CLOSE 시 Payment(PAID) 기록.
- 선불(미래): 자동 접수 구조에서는 "결제 완료 후에만 주문 생성"으로 처리 예정. 즉 PG 웹훅 검증 전에는 orders 레코드를 만들지 않거나 대기열에 노출하지 않는다. **자동 접수 후 결제 실패 취소** 문제는 PG 도입 시점에 상세 설계.
...
| 자동 접수 + 선결제 충돌 | 결제 확정 후에만 대기열 노출 (PG 도입 시 상세화) |
